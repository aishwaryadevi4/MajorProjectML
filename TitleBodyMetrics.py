...
fclean=open('cleanedTitleBody.txt','rt',encoding="utf-8")
tag_rows=ftag.read().split('\n')[:150000]
# X-> cleaned TitleBody
X=fclean.read().split('\n')[:150000]
# Y-> most repeated tag for that title and body, so basically most probable tag for the given title and body
Y = [[] for i in range(len(X))]
classifier = Pipeline([
		     ('vectorizer', CountVectorizer(analyzer='word',stop_words = s, lowercase = True, ngram_range=(1, 2))),
		     ('tfidf', TfidfTransformer()),
		     ('clf', OneVsRestClassifier(LinearSVC(C=1.5, penalty='l2', dual=True), n_jobs = -2))])
